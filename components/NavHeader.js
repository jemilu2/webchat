/*
Navigation for Left Pane
*/

import '../static/styles/tailwind.css';
import PropTypes from 'prop-types';
import Avatar from './Avatar';
import MoreButton from './MoreButton';

const ChatButton = (props) => {
    return (
        <button className="h-8 w-8 p-1">
        <svg xmlns="http://www.w3.org/2000/svg" fill="grey" viewBox="0 0 20 20">
            <path d="M10 15l-4 4v-4H2a2 2 0 0 1-2-2V3c0-1.1.9-2 2-2h16a2 2 0 0 1 2 2v10a2 2 0 0 1-2 2h-8zM5 7v2h2V7H5zm4 0v2h2V7H9zm4 0v2h2V7h-2z"/>
        </svg>
        </button>
    );
}

const StatusButton = (props) => {
    return (
        <button className="h-8 w-8 p-1">
            <svg xmlns="http://www.w3.org/2000/svg" fill="grey" viewBox="0 0 20 20">
                <path d="M10 20a10 10 0 1 1 0-20 10 10 0 0 1 0 20zm7.75-8a8.01 8.01 0 0 0 0-4h-3.82a28.81 28.81 0 0 1 0 4h3.82zm-.82 2h-3.22a14.44 14.44 0 0 1-.95 3.51A8.03 8.03 0 0 0 16.93 14zm-8.85-2h3.84a24.61 24.61 0 0 0 0-4H8.08a24.61 24.61 0 0 0 0 4zm.25 2c.41 2.4 1.13 4 1.67 4s1.26-1.6 1.67-4H8.33zm-6.08-2h3.82a28.81 28.81 0 0 1 0-4H2.25a8.01 8.01 0 0 0 0 4zm.82 2a8.03 8.03 0 0 0 4.17 3.51c-.42-.96-.74-2.16-.95-3.51H3.07zm13.86-8a8.03 8.03 0 0 0-4.17-3.51c.42.96.74 2.16.95 3.51h3.22zm-8.6 0h3.34c-.41-2.4-1.13-4-1.67-4S8.74 3.6 8.33 6zM3.07 6h3.22c.2-1.35.53-2.55.95-3.51A8.03 8.03 0 0 0 3.07 6z"/>
            </svg>
        </button>
    );
}



const Nav = (props) => {

    return (
        <div id="left-nav" className="flex shadow justify-between h-16">
            <div><Avatar image={props.image} /></div>
            <div className="flex flex-row">
            <div className="my-2 mx-1"><StatusButton /></div>
            <div className="my-2 mx-1"><ChatButton /></div>
            <div className="my-2 mx-1"><MoreButton /></div>
            </div>

            <style jsx>{`
                #left-nav {
                    background-color: #f7f7f7;
                    max-width:20rem;
                    //border: 1px solid red;
                }
            `}
            </style>
        </div>
    );

}

Nav.propTypes = {
    image : PropTypes.string,
}

export default Nav;